...
5. the single active brief under `tools/automation/briefs/active/`
6. the relevant prototype README, tests, fixtures, evidence, and implementation findings

## Authority hierarchy

1. The operator-authorized active brief controls the exact implementation slice.
...
A prototype packet is not a canonical Cadence schema. A recommendation is not authorization.
...
- **ChatGPT reasoning layer:** re-anchor, compare repository evidence, author or refine bounded briefs and documentation, interpret findings, and prepare safe handoffs.
...
Keep one active implementation brief by default. Do not interpret an implementation finding or next-slice idea as authorization.
...
- Keep baseline, scenario, authorization, revalidation, and applied revision structurally distinct.
- Use explicit fixture eligibility; do not infer staffing authority.
- Preserve PTO and dependencies as hard constraints in Slice 0.
- Expose cross-project displacement and overtime cost.
- Bind authorization to the exact scenario version and change-set digest.
- Reject baseline revision mismatch; Slice 0 does not silently rebase authorization.
...
- a change would define canonical shared project, task, schedule, or constraint semantics;
...
- active brief and authority status;
- files changed;
